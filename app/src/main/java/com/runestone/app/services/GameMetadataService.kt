...
class GameMetadataService(private val context: Context) {
     ...
     */
    suspend fun fetchMetadata(gameTitle: String, engineType: String? = null): GameMetadata? {
    ...
    private fun fetchFromRAWG(gameTitle: String): GameMetadata? {
        return try {
            ...
            val url = URL("https://api.rawg.io/api/games?search=$encodedTitle&page_size=1")
            ...
            if (connection.responseCode == 200) {
                ...
                if (results != null && results.length() > 0) {
                    ...
                    GameMetadata(
                        title = game.optString("name") ?: gameTitle,
                        description = game.optString("description_raw").takeIf { it.isNotEmpty() },
                        coverUrl = game.optString("background_image").takeIf { it.isNotEmpty() },
                        screenshots = screenshots,
                        releaseDate = game.optString("released").takeIf { it.isNotEmpty() },
                        developer = null, // Would need additional API call
                        publisher = null, // Would need additional API call
                        genres = genres,
